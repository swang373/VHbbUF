...
std::vector<std::string> GetSelExpressions(const std::string id) {
	...
	//values[0] = "ZnunuHighPt_VH := (Vtype==4)"; // <-- leave blank for now
        ...
        //values[0] = "(Vtype==4) && HmassReg<250 &&  met_pt>150 &&  max(Jet_btagCSV[hJCidx[0]],Jet_btagCSV[hJCidx[1]])>0.814 && min(Jet_btagCSV[hJCidx[0]],Jet_btagCSV[hJCidx[1]])>0.423"; // Use this line for SkimClassification.C
        values[0] = "ZnunuHighPt_VH := (Vtype==4) && HmassReg<250 &&  met_pt>150 &&  max(Jet_btagCSV[hJCidx[0]],Jet_btagCSV[hJCidx[1]])>0.814 && min(Jet_btagCSV[hJCidx[0]],Jet_btagCSV[hJCidx[1]])>0.423"; // Use this line for TrimTree.C
        // swang373, 08.07.2015
