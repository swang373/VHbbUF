...
for tag in TAGS:
  inf      = TFile.Open('TMVAReg.root')
  tree     = inf.Get('TestTree')
  hRes   = TProfile('hRes_'+tag,'hRes_'+tag,4,0,400, -100, 100, "")
  if tag == 'baseline':
#    tree.Draw('genPt>>hPtAll_'+tag, 'genPt>5')
#    tree.Draw('genPt>>hPtPass_'+tag,'pass && genMatchDR<0.2')
    tree.Draw('(breg_pt - Jet_mcPt_hJCidx_):Jet_mcPt_hJCidx_>>hRes_'+tag, "",  "profs" )
  else:
    tree.Draw('(BDTG - Jet_mcPt_hJCidx_):Jet_mcPt_hJCidx_>>hRes_'+tag, "",  "profs" )
  resPt.append(hRes)
