...
public:
    ...
    void check() const {
        ...
        assert(VV       != 0 && VV      ->GetEntriesFast() > 0);
               ...
               TString options="printStat:printCard:plotData:plotLog:plotSig")
{
    ...
    ev->VV->Project("VV_0", var, cutmc);
    std::clog << "... DONE: project VV_0." << std::endl;
    ...
    // Do the rebinning
    TH1F * hZH, * hWH, * hWjLF, * hWjHF, * hZjLF, * hZjHF, * hTT, * hs_Top, * hVV, * hQCD,
    ...
    if (syst == "NONE") {
        ...
        gPad->Print("plots/"+plotname+".png");
        gPad->Print("plots/"+plotname+".pdf");
...
EventsJ9 * Read(bool isControlRegion, const TCut cutallmc, const TCut cutalldata)
{
    ...
    ev->VV = (TTree *) VV.CopyTree(cutallmc);
    std::clog << "... DONE: VV copy tree." << std::endl;
...
void BDTShapeJ9(Int_t nbins=500, Int_t newnbins=16, Double_t rebinerrorf=0.35, bool pqrs=false, bool isControlRegion=false)
{
    ...
    for (Int_t ireg=begin; ireg<end; ireg++) {    
        ...
        TString cutmc=g_cutmc, cutdata=g_cutdata, ssyst=Form("%i",0);
        cutmc.ReplaceAll("ISYST",ssyst);
        cutdata.ReplaceAll("ISYST",ssyst);
        
        MakePlots(ev, "mindPhiMETJet_dPhi", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[ireg], "min |#Delta #varphi|(pfMET, cj30)", 17, -0.1, 3.4, 17, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
        MakePlots(ev, "min(Min$(abs(deltaPhi(METtype1corr.phi,hJet_phi))),1000*(Sum$(aJet_pt>25 && abs(aJet_eta)<4.5 && aJet_id && aJet_puJetIdL>0)==0)+Min$(abs(deltaPhi(METtype1corr.phi,aJet_phi))+1000*(aJet_pt<=25 || abs(aJet_eta)>=4.5 || !aJet_id || aJet_puJetIdL<=0)))", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[ireg], "min #Delta#varphi(pfMET, j25)", 17, -0.1, 3.4, 17, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
        MakePlots(ev, "min(0+Min$(abs(deltaPhi(METtype1corr.phi,hJet_phi))),1000*(Sum$(aJet_pt>20 && abs(aJet_eta)<2.5)==0)+Min$(abs(deltaPhi(METtype1corr.phi,aJet_phi))+1000*(aJet_pt<=20 || abs(aJet_eta)>=2.5)))", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[ireg], "min #Delta#varphi(pfMET, cj20)", 17, -0.1, 3.4, 17, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
        
        MakePlots(ev, "naJets_Znn", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[ireg], "# of add. j30", 8, 0, 8, 8, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:plotData:!plotLog:plotSig");
        MakePlots(ev, "Sum$(aJet_pt>25 && abs(aJet_eta)<4.5 && aJet_id && aJet_puJetIdL>0)", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[ireg], "# of add. j25", 8, 0, 8, 8, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
        MakePlots(ev, "Sum$(aJet_pt>20 && abs(aJet_eta)<2.5)", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[ireg], "# of add. cj20", 8, 0, 8, 8, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
        
        MakePlots(ev, "METtype1corr.sig", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[ireg], "fancy pfMET significance", 20, 0., 200., 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
        MakePlots(ev, "METtype1corr.et/sqrt(METtype1corr.sumet)", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[ireg], "pfMET / sqrt(sumET)", 20, 0., 20., 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
    ...
    MakePlots(ev, "mindPhiMETJet_dPhi", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[0], "min |#Delta #varphi|(pfMET, cj30)", 17, -0.1, 3.4, 17, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
    MakePlots(ev, "min(Min$(abs(deltaPhi(METtype1corr.phi,hJet_phi))),1000*(Sum$(aJet_pt>25 && abs(aJet_eta)<4.5 && aJet_id && aJet_puJetIdL>0)==0)+Min$(abs(deltaPhi(METtype1corr.phi,aJet_phi))+1000*(aJet_pt<=25 || abs(aJet_eta)>=4.5 || !aJet_id || aJet_puJetIdL<=0)))", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[0], "min #Delta#varphi(pfMET, j25)", 17, -0.1, 3.4, 17, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
    MakePlots(ev, "min(0+Min$(abs(deltaPhi(METtype1corr.phi,hJet_phi))),1000*(Sum$(aJet_pt>20 && abs(aJet_eta)<2.5)==0)+Min$(abs(deltaPhi(METtype1corr.phi,aJet_phi))+1000*(aJet_pt<=20 || abs(aJet_eta)>=2.5)))", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[0], "min #Delta#varphi(pfMET, cj20)", 17, -0.1, 3.4, 17, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
    
    MakePlots(ev, "naJets_Znn", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[0], "# of add. j30", 8, 0, 8, 8, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:plotData:!plotLog:plotSig");
    MakePlots(ev, "Sum$(aJet_pt>25 && abs(aJet_eta)<4.5 && aJet_id && aJet_puJetIdL>0)", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[0], "# of add. j25", 8, 0, 8, 8, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
    MakePlots(ev, "Sum$(aJet_pt>20 && abs(aJet_eta)<2.5)", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[0], "# of add. cj20", 8, 0, 8, 8, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
    
    MakePlots(ev, "METtype1corr.sig", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[0], "fancy pfMET significance", 20, 0., 200., 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
    MakePlots(ev, "METtype1corr.et/sqrt(METtype1corr.sumet)", cutmc.Data(), cutdata.Data(), g_systematics[0], g_regions[0], "pfMET / sqrt(sumET)", 20, 0., 20., 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
...
/*
    ...
    /// Loop over systematics
    for (Int_t isyst = 0; isyst < nsyst; isyst++) {
        newnbins = 8;  // FIXME: currently it's hardcoded
        rebinerrorf = 0.35;
