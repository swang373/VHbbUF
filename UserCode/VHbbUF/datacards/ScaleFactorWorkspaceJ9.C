...
const Int_t jmax            = 9;
const Int_t nsyst           = 13 + 2 * 2;  // plus ZjModel, TTModel
const TString strategy      = "real";  // choose from: "real", "blind", "injectsignal"
...
const TString g_pdfname     = "vhbb_Znn_SF_$CHANNEL_TH1.pdf";
...
void ScaleFactorWorkspaceJ9()
{
    ...
    for (UInt_t ichan = 0; ichan < channels.size(); ichan++) {
        ...
        if (channel == "ZnunuLowCSV") {
            MakeWorkspace(channel + "_ZjHF");
            MakeWorkspace(channel + "_WjHF");
        } else {
            for (Int_t ireg=begin; ireg<end; ireg++) {
                MakeWorkspace(channel + "_" + g_regions[ireg]);
            }
        }
    }
