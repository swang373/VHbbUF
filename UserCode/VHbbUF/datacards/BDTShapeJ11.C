...
// TODO:
// - Veto pixel movement runs. new lumi 19.04, isBadHcalEvent [DONE]
// - Add trigger up/down shape [DONE]
// - Use VVHF = VVHF = WZ+HF, ZZ+HF and VVLF = WZ+LF, ZZ+LF, WW
// - Change normalization in nuisance histograms
// - Add signal NNLO correction
// - Include Mjj cuts
...
const TCut    g_cutalldata  = "(!(207883<=EVENT.run && EVENT.run<=208307))";  // used in CopyTree()
...
const TString g_varslice    = Form("slice(BDTregular_%i[ISYST], (BDTtt_%i[ISYST]>-0.6), (BDTvjlf_%i[ISYST]>-0.5), (BDTzz_%i[ISYST]>-0.3))", massH, massH, massH, massH);
...
const TString g_dcname      = "vhbb_Znn_J11_$CHANNEL_8TeV.txt";
const TString g_wsname      = "vhbb_Znn_J11_$CHANNEL_8TeV.root";
const TString g_rootname    = "vhbb_Znn_J11_$CHANNEL_TH1.root";
...
const double g_scalefactors_lnN_ZnunuHighPt[7] = {
  //1.03, 1.30, 1.40, 1.03, 1.07, 1.10, 1.03
    1.03, 1.30, 1.40, 1.04, 1.09, 1.12, 1.04
};
...
const double g_scalefactors_QCD[3] = {0.005, 0.05, 0.03};
////////////////////////////////////////////////////////////////////////////////
/// END Configuration                                                        ///
////////////////////////////////////////////////////////////////////////////////
...
void MakePlots(const EventsJ11 * ev, const TString var, 
               ...
               TString options="printStat:printCard:plotData:plotLog:plotSig")
{
    ...
    // Apply slope
    double WJSlopeErr = 0.0015;
    double ZJSlopeErr = 0.0025;
    double WJSlope = -0.00;  // -1.64
    double ZJSlope = -1.00;  // -4.04
    TCut cutwjslope = Form("VaryVptSlope(genW.pt, %f, 150)", WJSlope * WJSlopeErr);
    TCut cutzjslope = Form("VaryVptSlope(genZ.pt, %f, 130)", ZJSlope * ZJSlopeErr);
    ...
    if (printCard && syst == "NONE") {
        ...
        dc << "lumi_8TeV                           lnN    1.05  1.05  -     -     -     -     -     -     -     1.05  1.05  1.05 " << std::endl;
        ...
        dc << "CMS_vhbb_VV                         lnN    -     -     -     -     -     -     -     -     -     -     1.30  -    " << std::endl;
        dc << "#CMS_vhbb_MET_nojets                lnN    1.03  1.03  -     -     -     -     -     -     -     1.03  1.03  -    " << std::endl;
        ...
        dc << "CMS_vhbb_statVV_" << channel_8TeV << "    shape  -     -     -     -     -     -     -     -     -     -     1.00  -    " << std::endl;
        ...
        dc << "CMS_vhbb_eff_b_SIG                  shape  1.00  1.00  -     -     -     -     -     -     -     -     -     -     -    " << std::endl;
        dc << "CMS_vhbb_eff_b                      shape  -     -     1.00  1.00  1.00  1.00  1.00  1.00  1.00  1.00  1.00  1.00  -    " << std::endl;
    ...
    if (syst == "NONE") {
        ...
        hs->SetMaximum(ymax + ymax / 2.0 + (ymax>1 ? sqrt(ymax) : 0.));
        if (plotLog)
    ...
    if (writeRoot) {
        ...
        if (syst == "NONE") {
            ...
            cutwjslope = Form("VaryVptSlope(genW.pt, %f, 130)", (WJSlope + 1.0) * WJSlopeErr);
            cutzjslope = Form("VaryVptSlope(genZ.pt, %f, 130)", (ZJSlope + 1.0) * ZJSlopeErr);
            ...
            delete hWj0b_slopeUp;
            delete hWj1b_slopeUp;
            delete hWj2b_slopeUp;
            delete hZj0b_slopeUp;
            delete hZj1b_slopeUp;
            delete hZj2b_slopeUp;
            delete hWj0b_slopeDown;
            delete hWj1b_slopeDown;
            delete hWj2b_slopeDown;
            delete hZj0b_slopeDown;
            delete hZj1b_slopeDown;
            delete hZj2b_slopeDown;
        }
        out->Close();
        delete out;
    }
...
void BDTShapeJ11(int nbins=500, long long newnbins=15, double rebinerrorf=0.35, bool freerebin=false, bool isControlRegion=false)  // 2000, 12121212 for multi-BDT
{
    gROOT->LoadMacro("tdrstyle.C");
    gROOT->ProcessLine("setTDRStyle()");
    gROOT->LoadMacro("HelperFunctions.h");  // FIXME
    gROOT->LoadMacro("HelperBDTShape.h");  // FIXME
    ...
    for (int ireg=begin; ireg<end; ireg++) {
        if (ireg==0) {  // signal region
            ...
            for (int isyst = 0; isyst < nsyst; isyst++) {
                ...
                TString var = (!makeslice) ? g_var : g_varslice;
                TString ssyst=Form("%i",isyst);
                var.ReplaceAll("ISYST",ssyst);
                
                TCut cutmc_test   = Form("2.0 * weightsMC[%i] * (selectFlags[0][%i]) * triggercorr2012ABCD( (triggerFlags[42]==1 || triggerFlags[39]==1), (triggerFlags[41]==1), METtype1corr.et, max(hJet_csv_nominal[0],hJet_csv_nominal[1]), naJets_Znn ) * 19040/19624", isyst, isyst); // FIXME
                if (g_systematics[isyst] == "CMS_vhbb_trigger_MET_Znunu_8TeVUp")
                    cutmc_test   = Form("2.0 * weightsMC[%i] * (selectFlags[0][%i]) * triggercorr2012ABCD_up( (triggerFlags[42]==1 || triggerFlags[39]==1), (triggerFlags[41]==1), METtype1corr.et, max(hJet_csv_nominal[0],hJet_csv_nominal[1]), naJets_Znn ) * 19040/19624", isyst, isyst); // FIXME
                TCut cutdata_test = Form("selectFlags[0][%i]", isyst);
                
                MakePlots(ev, var, cutmc_test, cutdata_test, g_systematics[isyst], g_regions[ireg], "BDT", nbins, g_xlow, g_xup, newnbins, rebinerrorf, scalefactors_lnN, "printStat:printCard:writeRoot:!plotData:plotLog:plotSig");
            }
        }
        ...
        if (g_manyplots) {
            ...
            ev->set_scalefactors(&scalefactors[0]);
            TCut cutmc_ctrl = Form("weightsMC[0] * (selectFlags[%i][0]) * triggercorr2012ABCD( (triggerFlags[42]==1 || triggerFlags[39]==1), (triggerFlags[41]==1), METtype1corr.et, max(hJet_csv_nominal[0],hJet_csv_nominal[1]), naJets_Znn ) * 19040/19624", ireg);  // FIXME
            TCut cutdata_ctrl = Form("(selectFlags[%i][0])", ireg);
            TCut cutmass_ctrl = "";
            if (ireg==0) {  // signal region
                cutmc_ctrl = Form("2.0 * weightsMC[0] * (selectFlags[%i][0]) * triggercorr2012ABCD( (triggerFlags[42]==1 || triggerFlags[39]==1), (triggerFlags[41]==1), METtype1corr.et, max(hJet_csv_nominal[0],hJet_csv_nominal[1]), naJets_Znn ) * 19040/19624", ireg);  // FIXME
                cutmass_ctrl = "(HmassReg<100 || 140<HmassReg)";
            }
            
            if (ireg!=0) {  // control region
                MakePlots(ev, Form("BDTregular_%i[0]", massH), cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "BDT", nbins, g_xlow, g_xup, newnbins, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:plotLog:plotSig");
            }
            MakePlots(ev, "HmassReg", cutmc_ctrl*cutmass_ctrl, cutdata_ctrl*cutmass_ctrl, g_systematics[0], g_regions[ireg], "H mass [GeV]", 23, 20, 250, 23, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");  // apply mass veto
            MakePlots(ev, "HptReg", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H p_{T} [GeV]", 23, 115, 460, 23, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "max(hJet_ptReg[0],hJet_ptReg[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H j1 p_{T} [GeV]", 20, 50, 350, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "min(hJet_ptReg[0],hJet_ptReg[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H j2 p_{T} [GeV]", 13, 15, 210, 13, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "deltaR(hJet_eta[0], hJet_phi[0], hJet_eta[1], hJet_phi[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "#Delta R(j1,j2)", 14, 0, 3.5, 14, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "abs(hJet_eta[0]-hJet_eta[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "|#Delta #eta|(j1,j2)", 14, 0, 3.5, 14, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            ...
            MakePlots(ev, "METtype1corr.et", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "Type-1 corr. pfMET [GeV]", 20, 155, 455, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "HMETdPhi", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "|#Delta #varphi|(H, pfMET)", 16, 1.6, 3.2, 16, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "mindPhiMETJet_dPhi", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "min |#Delta #varphi|(pfMET, j25)", 17, -0.1, 3.3, 17, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "max(hJet_csv_nominal[0],hJet_csv_nominal[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "CSV_{max}(j1,j2)", 30, 0.0, 1.08, 30, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "min(hJet_csv_nominal[0],hJet_csv_nominal[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "CSV_{min}(j1,j2)", 30, 0.0, 1.08, 30, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            ...
            MakePlots(ev, "MaxIf$(max(aJet_csv_nominal,0), aJet_pt>20 && abs(aJet_eta)<2.5)", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "CSV_{max}(add. cj20)", 30, 0.0, 1.08, 30, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "min(mindRHJet_dR,5.653)", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "min #DeltaR(H, add. j25)", 23, 0, 5.75, 23, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            
            // Bkg specific BDT
            MakePlots(ev, Form("BDTtt_%i[0]", massH), cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "BDTtt", 15, -1, 1, 15, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:plotLog:plotSig");
            MakePlots(ev, Form("BDTvjlf_%i[0]", massH), cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "BDTvjlf", 15, -1, 1, 15, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:plotLog:plotSig");
            MakePlots(ev, Form("BDTzz_%i[0]", massH), cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "BDTzz", 15, -1, 1, 15, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:plotLog:plotSig");
            
            // QCD check
            MakePlots(ev, "METtype1corr.et/sqrt(METtype1corr.sumet)", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "Type-1 corr. pfMET/sqrt(sumET) [GeV]", 20, 0, 18, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "METtype1corr.sig", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "Type-1 corr. pfMET significance", 18, 0, 360, 18, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "nPVs", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "# of primary vertices", 20, 0, 40, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "mindPhiMETCtrJet_dPhi", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "min |#Delta #varphi|(pfMET, cj20)", 17, -0.1, 3.3, 17, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            ...
            // FatH.filteredmass
            MakePlots(ev, "FatH.filteredmass * (nfathFilterJets>0 && FatH.FatHiggsFlag==1)", cutmc_ctrl*cutmass_ctrl, cutdata_ctrl*cutmass_ctrl, g_systematics[0], g_regions[ireg], "H_{fj} mass [GeV]", 25, 0, 250, 25, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");  // apply mass veto
            ...
            // FatHmassReg
            MakePlots(ev, "FatHmassReg * (nfathFilterJets>0 && FatH.FatHiggsFlag==1)", cutmc_ctrl*cutmass_ctrl, cutdata_ctrl*cutmass_ctrl, g_systematics[0], g_regions[ireg], "H_{fj} mass [GeV]", 25, 0, 250, 25, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");  // apply mass veto
            ...
            // Out of the box
            MakePlots(ev, "hJet_pt[0]", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H j1 p_{T} [GeV] (no regression)", 20, 50, 350, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "hJet_pt[1]", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H j2 p_{T} [GeV] (no regression)", 13, 15, 210, 13, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "H.pt", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H p_{T} [GeV] (no regression)", 23, 115, 460, 23, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "MET.et", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "Type-1 corr. pfMET [GeV] (no JEC update)", 20, 155, 455, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "V.pt", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "V p_{T} [GeV] (no JEC update)", 20, 155, 455, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "Sum$(aJet_pt>30 && abs(aJet_eta)<4.5 && aJet_id==1 && aJet_puJetIdL>0)", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "# of add. jets (p_{T} > 30 GeV)", 8, 0, 8, 8, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:plotData:!plotLog:plotSig");
        }
    ...
    if (!freerebin) {  // use hardcoded binning choice
        newnbins = (!makeslice) ? 15 : 11111111;
        rebinerrorf = 0.35;
    }
    ...
    for (int ireg=begin; ireg<end; ireg++) {
        ...
        if (g_manyplots) {
            ...
            MakePlots(ev, "abs(hJet_eta[0]-hJet_eta[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "|#Delta #eta|(j1,j2)", 14, 0, 3.5, 14, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "min(abs(deltaPullAngle),pi)", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "|#Delta #theta_{pull}|(j1,j2)", 16, 0, 3.2, 16, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "METtype1corr.et", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "Type-1 corr. pfMET [GeV]", 12, 120, 180, 12, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "HMETdPhi", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "|#Delta #varphi|(H, pfMET)", 16, 1.6, 3.2, 16, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "mindPhiMETJet_dPhi", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "min |#Delta #varphi|(pfMET, j25)", 17, -0.1, 3.3, 17, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "max(hJet_csv_nominal[0],hJet_csv_nominal[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "CSV_{max}(j1,j2)", 30, 0.0, 1.08, 30, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "min(hJet_csv_nominal[0],hJet_csv_nominal[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "CSV_{min}(j1,j2)", 30, 0.0, 1.08, 30, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            ...
            // QCD check
            MakePlots(ev, "METtype1corr.et/sqrt(METtype1corr.sumet)", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "Type-1 corr. pfMET/sqrt(sumET) [GeV]", 20, 0, 12, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "METtype1corr.sig", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "Type-1 corr. pfMET significance", 15, 0, 270, 15, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "nPVs", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "# of primary vertices", 20, 0, 40, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "mindPhiMETCtrJet_dPhi", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "min |#Delta #varphi|(pfMET, cj20)", 17, -0.1, 3.3, 17, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            ...
            MakePlots(ev, "hJet_pt[1]", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H j2 p_{T} [GeV] (no regression)", 13, 15, 210, 13, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "H.pt", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H p_{T} [GeV] (no regression)", 23, 115, 460, 23, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "MET.et", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "Type-1 corr. pfMET [GeV] (no JEC update)", 12, 120, 180, 12, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "V.pt", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "V p_{T} [GeV] (no JEC update)", 20, 120, 220, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "Sum$(aJet_pt>30 && abs(aJet_eta)<4.5 && aJet_id==1 && aJet_puJetIdL>0)", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "# of add. jets (p_{T} > 30 GeV)", 8, 0, 8, 8, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:plotData:!plotLog:plotSig");
        }
    }  // end loop over regions
    delete ev;
    ...
    std::clog << "\n*** " << channel << " ***\n" << std::endl;
    if (!freerebin) {  // use hardcoded binning choice
        newnbins = (!makeslice) ? 13 : 10101010;
        rebinerrorf = 0.35;
    }
    ...
    for (int ireg=begin; ireg<end; ireg++) {
        ...
        if (g_manyplots) {
            ...
            MakePlots(ev, "HmassReg", cutmc_ctrl*cutmass_ctrl, cutdata_ctrl*cutmass_ctrl, g_systematics[0], g_regions[ireg], "H mass [GeV]", 23, 20, 250, 23, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");  // apply mass veto
            MakePlots(ev, "HptReg", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H p_{T} [GeV]", 24, 45, 405, 24, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "max(hJet_ptReg[0],hJet_ptReg[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H j1 p_{T} [GeV]", 20, 15, 315, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "min(hJet_ptReg[0],hJet_ptReg[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H j2 p_{T} [GeV]", 13, 15, 210, 13, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "deltaR(hJet_eta[0], hJet_phi[0], hJet_eta[1], hJet_phi[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "#Delta R(j1,j2)", 14, 0, 3.5, 14, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "abs(hJet_eta[0]-hJet_eta[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "|#Delta #eta|(j1,j2)", 14, 0, 3.5, 14, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "min(abs(deltaPullAngle),pi)", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "|#Delta #theta_{pull}|(j1,j2)", 16, 0, 3.2, 16, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "METtype1corr.et", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "Type-1 corr. pfMET [GeV]", 10, 90, 140, 10, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "HMETdPhi", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "|#Delta #varphi|(H, pfMET)", 16, 1.6, 3.2, 16, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "mindPhiMETJet_dPhi", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "min |#Delta #varphi|(pfMET, j25)", 17, -0.1, 3.3, 17, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "max(hJet_csv_nominal[0],hJet_csv_nominal[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "CSV_{max}(j1,j2)", 30, 0.0, 1.08, 30, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "min(hJet_csv_nominal[0],hJet_csv_nominal[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "CSV_{min}(j1,j2)", 30, 0.0, 1.08, 30, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            ...
            // QCD check
            MakePlots(ev, "METtype1corr.et/sqrt(METtype1corr.sumet)", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "Type-1 corr. pfMET/sqrt(sumET) [GeV]", 20, 0, 10, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "METtype1corr.sig", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "Type-1 corr. pfMET significance", 12, 0, 180, 12, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "nPVs", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "# of primary vertices", 20, 0, 40, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "mindPhiMETCtrJet_dPhi", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "min |#Delta #varphi|(pfMET, cj20)", 17, -0.1, 3.3, 17, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            
            // Out of the box
            MakePlots(ev, "hJet_pt[0]", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H j1 p_{T} [GeV] (no regression)", 20, 15, 315, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "hJet_pt[1]", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H j2 p_{T} [GeV] (no regression)", 13, 15, 210, 13, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "H.pt", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H p_{T} [GeV] (no regression)", 24, 45, 405, 24, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "MET.et", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "Type-1 corr. pfMET [GeV] (no JEC update)", 10, 90, 140, 10, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "V.pt", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "V p_{T} [GeV] (no JEC update)", 20, 90, 190, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "Sum$(aJet_pt>30 && abs(aJet_eta)<4.5 && aJet_id==1 && aJet_puJetIdL>0)", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "# of add. jets (p_{T} > 30 GeV)", 8, 0, 8, 8, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:plotData:!plotLog:plotSig");
        }
    }  // end loop over regions
    delete ev;
...
/*
    ...
    std::clog << "\n*** " << channel << " ***\n" << std::endl;
    if (!freerebin) {  // use hardcoded binning choice
        newnbins = (!makeslice) ? 11 : 10101010;
        rebinerrorf = 0.35;
    }
