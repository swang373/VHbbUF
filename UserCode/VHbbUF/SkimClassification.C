...
void SkimClassification(TString process="ZnnH125")
{
    ...
    TString indir   = "skim_ZnnH_baseline/";
    TString outdir  = "skim_ZnnH_classification/";
    ...
    // Get selections
    const std::vector < std::string > & selExpressions = GetZnunuSelExpressions();
    const UInt_t nsels = 3;  // ZnunuHighPt, ZnunuLowPt, ZnunuLowCSV
    assert(nsels == selExpressions.size());
    // even-number events for training, odd-number events for testing
    TCut evenselection = "EVENT.event %2 == 0";
    TCut oddselection  = "EVENT.event %2 == 1";
