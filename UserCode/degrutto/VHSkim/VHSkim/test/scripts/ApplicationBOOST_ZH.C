...
if(channel == 4)   filename = new TString("TMVA-Ts.root");
if(channel == 5)   filename = new TString("TMVA-Tt.root");
if(channel == 6)   filename = new TString("TMVA-TtW.root");
if(channel == 7)   filename = new TString("TMVA-WW.root");
if(channel == 8)   filename = new TString("TMVA-WZ.root");
if(channel == 9)   filename = new TString("TMVA-WJ.root");
if(channel == 10)   filename = new TString("TMVA-ZJL.root");
if(channel == 11)   filename = new TString("TMVA-ZJH.root");
}
   //---------------------------------------------------------------
...
//   reader->AddVariable( "zmmMass"      ,  &zmmMass);
   reader->AddVariable( "zmmPt"        ,  &zmmPt);
   reader->AddVariable( "hjjzmmdPhi"   ,  &hjjzmmdPhi);
// if(lng)  reader->AddVariable( "Jet1.pt"       ,  &Jet1.pt);
// if(lng)  reader->AddVariable( "Jet1.phi"      ,  &Jet1.phi);
//   reader->AddVariable( "Jet2.pt"      ,   &Jet2.pt);
//if(lng)  reader->AddVariable( "Jet2.phi"      ,  &Jet2.phi);
...
if(channel == 0)   TString fname = "./B-42-ZllHbb115.root";   
if(channel == 1)   TString fname = "./B-42-SingleMu1fb.root";   
if(channel == 2)   TString fname = "./B-42-TTbar.root";   
if(channel == 3)   TString fname = "./B-42-ZZ.root";   
if(channel == 4)   TString fname = "./B-42-T-s.root";
if(channel == 5)   TString fname = "./B-42-T-t.root";
if(channel == 6)   TString fname = "./B-42-T-W.root";
if(channel == 7)   TString fname = "./B-42-WW.root";
if(channel == 8)   TString fname = "./B-42-WZ.root";
if(channel == 9)   TString fname = "./B-42-WJetsMad.root";
if(channel == 10)   TString fname = "./B-42-ZJetsMad.root";
if(channel == 11)   TString fname = "./B-42-ZJets100Mad.root";
   ...
   theTree->SetBranchAddress( "numJets"   ,  &numJets);
   theTree->SetBranchAddress( "PUweight"   ,  &PUweight);
   
   theTree->SetBranchAddress("eventInfo",      &_EventInfo);
   theTree->SetBranchAddress( "weightTrig"   ,  &weightTrig);
   theTree->SetBranchAddress("isIsoMu17", &isIso); 
   ...
   tree->Branch("Jet1"		,  &Jet1	    ,  "pt/F:eta/F:phi/F:csv/F:cosTheta/F:numTracksSV/I:chf/F:nhf:cef:nef:nch:nconstituents:flavour:genPt:genEta:genPhi:JECUnc/F");
   tree->Branch("Jet2"		,  &Jet2	    ,  "pt/F:eta/F:phi/F:csv/F:cosTheta/F:numTracksSV/I:chf/F:nhf:cef:nef:nch:nconstituents:flavour:genPt:genEta:genPhi:JECUnc/F");
   tree->Branch("H"		,  &H	            ,  "mass/F:pt/F:eta:phi/F");
   tree->Branch("zmmMass"  	,  &zmmMass      ,  "zmmMass/F"    );                                     	
   tree->Branch("zmmPt"  	,  &zmmPt        ,  "zmmPt/F"      );           	
   tree->Branch("hjjzmmdPhi"    ,  &hjjzmmdPhi   ,  "hjjzmmdPhi/F" );                
   tree->Branch("isIsoMu17"     ,  &isIso    , "isIsoMu17/b"); 
 
   tree->Branch("eventInfo",      &_EventInfo	    ,  "run/I:lumi/I:event/I");
   ...
   for (Long64_t ievt=0; ievt<theTree->GetEntries();ievt++) {
      ...
      deltaEta = fabs(Jet2.eta-Jet1.eta);
...
///    std::cout <<                 Jet1.csv              << std::endl;      
///    std::cout <<                 Jet2.csv                << std::endl;    
///    std::cout <<                 zmmMass                   << std::endl;
///    std::cout <<                 zmmPt                     << std::endl;
///    std::cout <<                 hjjzmmdPhi                  << std::endl; 
///    std::cout <<                 Jet1.pt                    << std::endl;
///    std::cout <<                 Jet1.phi                    << std::endl;
///    std::cout <<                 Jet2.pt                    << std::endl;
///    std::cout <<                 Jet2.phi                    << std::endl;
