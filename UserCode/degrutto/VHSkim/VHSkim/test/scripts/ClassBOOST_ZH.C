// @(#)root/tmva $Id: ClassBOOST_ZH.C,v 1.2 2011/12/14 14:50:10 madfish Exp $
/**********************************************************************************
...
//   factory->AddVariable( "maxcsv := max(hJet_csv[0],hJet_csv[1])"      ,  "CSV 2"     , "" 	, 'F' );
   ...
   factory->AddVariable( "H.mass"    ,  "H Mass"    , "GeV"	, 'F' );
   factory->AddVariable( "H.pt"      ,  "H PT"      , "GeV" 	, 'F' );
//   factory->AddVariable( "mincsv := min(hJet_csvCor[0],hJet_csvCor[1])"      ,  "CSV 1"     , ""		, 'F' );
//   factory->AddVariable( "maxcsv := max(hJet_csvCor[0],hJet_csvCor[1])"      ,  "CSV 2"     , "" 	, 'F' );
   factory->AddVariable( "hJet_csvCor[0]"      ,  "CSV 1"     , ""		, 'F' );
   factory->AddVariable( "hJet_csvCor[1]"      ,  "CSV 2"     , "" 	, 'F' );
   ...
   factory->AddVariable( "numaddj := Sum$(aJet_pt > 20 & abs(aJet_eta) < 2.4)"      ,  "Num Add Jets"     , ""		, 'F' );
...
//      gSystem->Exec("wget http://root.cern.ch/files/tmva_class_example.root");
 ...
 TFile *inputBtt  =   TFile::Open("A-TTJets_TuneZ2_7TeV-madgraph-tauolaProcV1_Fall11_60M.root");
 TFile *inputBzz  =   TFile::Open("A-ZZ_TuneZ2_7TeV_pythia6_tauolaProcV2_Fall11.root"                 );
 TFile *inputts   =   TFile::Open("A-T_TuneZ2_s-channel_7TeV-powheg-tauolaProcV2_Fall11.root"         );
 TFile *inputTt   =   TFile::Open("A-T_TuneZ2_t-channel_7TeV-powheg-tauolaProcV2_Fall11.root"         );
 TFile *inputtw   =   TFile::Open("A-T_TuneZ2_tW-channel-DR_7TeV-powheg-tauolaProcV2_Fall11.root"     );
 TFile *inputtbs   =  TFile::Open("A-Tbar_TuneZ2_s-channel_7TeV-powheg-tauolaProcV2_Fall11.root"      );
 TFile *inputTtb   =  TFile::Open("A-Tbar_TuneZ2_t-channel_7TeV-powheg-tauolaProcV2_Fall11.root"      );
 TFile *inputtbw   =  TFile::Open("A-Tbar_TuneZ2_tW-channel-DS_7TeV-powheg-tauolaProcV2_Fall11.root"  );
 TFile *inputww   =   TFile::Open("A-WW_TuneZ2_7TeV_pythia6_tauolaProcV1_Fall11.root"		      );
 TFile *inputwz   =   TFile::Open("A-WZ_TuneZ2_7TeV_pythia6_tauolaProcV2_Fall11.root"                 );
 TFile *inputwj   =   TFile::Open("A-WJetsToLNu_TuneZ2_7TeV-madgraph-tauolaProcV2_Fall11.root"        );
 TFile *inputBzj   =  TFile::Open("A-DYJetsToLL_TuneZ2_M-50_7TeV-madgraph-tauolaProcV2_Fall11.root"   );
 TFile *inputBzj2   = TFile::Open("A-DYJetsToLL_PtZ-100_TuneZ2_7TeV-madgraph-tauolaProcV1_Fall11.root");
...
/*
 ...
 TFile *inputtw   = TFile::Open("A-T_TuneZ2_tW-channel-DS_7TeV-powheg-tauola.root");
...
*/
 TFile *inputS = TFile::Open("A-ZH_ZToLL_HToBB_M-115_7TeV-powheg-herwigppProcV1_Fall11.root");
...
if(trainZJ && !trainTTbar && !trainZZ && !trainALL)  { factory->AddBackgroundTree(backgroundzj   , lumi/(lumiZJL)  );factory->AddBackgroundTree(backgroundzj2   , lumi/(lumiZJH));  factory->AddBackgroundTree(backgroundzjo   , lumi/(lumiZJL)  );factory->AddBackgroundTree(backgroundzjo2   , lumi/(lumiZJH));   }
if(!trainZJ && !trainTTbar && trainZZ && !trainALL)  factory->AddBackgroundTree(backgroundzz   , 1  );
if(!trainZJ && trainTTbar && !trainZZ && !trainALL)  factory->AddBackgroundTree(backgroundtt   , 1  );

if(!trainZJ && !trainTTbar && !trainZZ && trainALL)
{   
if(backgroundTtb->GetEntries() > 0)    factory->AddBackgroundTree(backgroundTtb,lumi/(lumiTtb/2.0));
if(backgroundtbs->GetEntries() > 0)    factory->AddBackgroundTree(backgroundtbs, lumi/(lumiTsb/2.0));
if(backgroundtbw->GetEntries() > 0)    factory->AddBackgroundTree(backgroundtbw, lumi/(lumiTtWb/2.0));
if(backgroundts->GetEntries() > 0) factory->AddBackgroundTree(backgroundts,lumi/(lumiTs/2.0));
if(backgroundTt->GetEntries() > 0) factory->AddBackgroundTree(backgroundTt,lumi/(lumiTt/2.0));
if(backgroundtw->GetEntries() > 0) factory->AddBackgroundTree(backgroundtw,lumi/(lumiTtW/2.0));
if(backgroundww->GetEntries() > 0) factory->AddBackgroundTree(backgroundww,lumi/(lumiWW/2.0));
if(backgroundwz->GetEntries() > 0) factory->AddBackgroundTree(backgroundwz,lumi/(lumiWZ/2.0));
if(backgroundwj->GetEntries() > 0) factory->AddBackgroundTree(backgroundwj,lumi/(lumiWJ/2.0));
if(backgroundzz->GetEntries() > 0) factory->AddBackgroundTree(backgroundzz   , lumi/(lumiZZ/2.0));
if(backgroundtt->GetEntries() > 0)   factory->AddBackgroundTree(backgroundtt   , lumi/(lumiTT/2.0));
if(backgroundzj->GetEntries() > 0) factory->AddBackgroundTree(backgroundzj,lumi/(lumiZJL/2.0));
if(backgroundzj2->GetEntries() > 0) factory->AddBackgroundTree(backgroundzj2,lumi/(lumiZJH/2.0));
...
//mycutb = "H.pt > 150 && hJet_csv[0] > 0.5 && hJet_csv[1] > 0.5 && Z.pt > 150"; 
   ...
   // factory->PrepareTrainingAndTestTree( mycuts,"NSigTrain=300:NBkgTrain=300:NSigTest=300:NBkgTest=300:SplitMode=Random:!V" );
    ...
    //ZJETS
 if(trainZJ && !trainTTbar && !trainZZ && !trainALL ) factory->PrepareTrainingAndTestTree( mycuts, mycutb,"nTrain_Signal=100:nTrain_Background=100:SplitMode=Block:NormMode=None:!V" );
 ...
 if(!trainZJ && !trainTTbar && trainZZ && !trainALL)   factory->PrepareTrainingAndTestTree( mycuts, mycutb,"nTrain_Signal=100:nTrain_Background=100:SplitMode=Block:NormMode=None:!V" );
   ...
   //TTbar
 if(!trainZJ && trainTTbar && !trainZZ && !trainALL)  factory->PrepareTrainingAndTestTree( mycuts, mycutb,"nTrain_Signal=5:nTrain_Background=5:SplitMode=Block:NormMode=None:!V" );
//NumEvents 
  //ALL
 /*here*/if(!trainZJ && !trainTTbar && !trainZZ && trainALL)  factory->PrepareTrainingAndTestTree( mycuts, mycutb,"nTrain_Signal=0:nTrain_Background=0:SplitMode=Random:NormMode=None:!V" );
   ...
   if (Use["BDT"])  // Adaptive Boost
      factory->BookMethod( TMVA::Types::kBDT, "BDT",
                       ...
                       "!H:!V:NTrees=100:nEventsMin=100:MaxDepth=3:BoostType=AdaBoost:AdaBoostBeta=0.3:SeparationType=GiniIndex:nCuts=35:PruneMethod=NoPruning" );
   ///*MJJD*/                     "!H:!V:NTrees=100:nEventsMin=150:MaxDepth=2:BoostType=AdaBoost:AdaBoostBeta=0.3:SeparationType=GiniIndex:nCuts=35:PruneMethod=NoPruning" );
   if (Use["BDTB"]) // Bagging
