...
std::vector<std::string> GetInputExpressionLabelsReg() {
    ...
    values[2] = "E_{T}(j);GeV;F";
    values[3] = "M_{T}(j);GeV;F";
    values[4] = "leading track p_{T}(j);GeV;F";
    values[5] = "sec vtx 3-d flight length(j);cm;F";
    values[6] = "sec vtx 3-d flight length error(j);cm;F";
    values[7] = "sec vtx mass(j);GeV;F";
    values[8] = "sec vtx p_{T}(j);GeV;F";
    values[9] = "charged electromagnetic frac(j);;F";
    values[10] = "# particles(j);;I";
    values[11] = "JEC uncertainty(j);GeV;F";
    values[12] = "p_{T, rel}(lep-in-j);GeV;F";
    values[13] = "p_{T}(lep-in-j);GeV;F";
    values[14] = "#DeltaR(lep-in-j);;F";
    return values;
}
...
std::vector<std::string> GetSelExpressions(const std::string id) {
    ...
    if (id == "ZnunuHighPt") {
        ...
        values[2] = "ZnunuHighPt_ZjHF := (Vtype==4) && METtype1corr.et>170 && HptReg>130 && max(hJet_ptReg[0],hJet_ptReg[1])>60 && min(hJet_ptReg[0],hJet_ptReg[1])>30 && max(hJet_csv_nominal[0],hJet_csv_nominal[1])>0.679 && min(hJet_csv_nominal[0],hJet_csv_nominal[1])>0.244 && (HmassReg<75 || 110<HmassReg) && HmassReg<250 && mindPhiMETJet_dPhi>0.5 && abs(deltaPhi(METtype1corr.phi,METnoPUCh.phi))<0.5 && abs(deltaPhi(H.phi,METtype1corr.phi))>2.0 && nalep_pt5_Znn==0 && hbhe && ecalFlag && cschaloFlag && hcallaserFlag && trackingfailureFlag && eebadscFlag && !isBadHcalEvent && hJet_puJetIdL[0]>0 && hJet_puJetIdL[1]>0 && HmassReg>0";
        ...
        values[4] = "ZnunuHighPt_WjHF := (Vtype==2||Vtype==3) && METtype1corr.et>170 && HptReg>130 && max(hJet_ptReg[0],hJet_ptReg[1])>60 && min(hJet_ptReg[0],hJet_ptReg[1])>30 && max(hJet_csv_nominal[0],hJet_csv_nominal[1])>0.679 && min(hJet_csv_nominal[0],hJet_csv_nominal[1])>0.244 && (HmassReg<75 || 110<HmassReg) && HmassReg<250 && mindPhiMETJet_dPhi>0.5 && abs(deltaPhi(H.phi,METtype1corr.phi))>2.0 && naJets_Znn==0 && hbhe && ecalFlag && cschaloFlag && hcallaserFlag && trackingfailureFlag && eebadscFlag && !isBadHcalEvent && hJet_puJetIdL[0]>0 && hJet_puJetIdL[1]>0 && HmassReg>0";
        values[5] = "ZnunuHighPt_TT := (Vtype==2||Vtype==3) && METtype1corr.et>170 && HptReg>130 && max(hJet_ptReg[0],hJet_ptReg[1])>60 && min(hJet_ptReg[0],hJet_ptReg[1])>30 && max(hJet_csv_nominal[0],hJet_csv_nominal[1])>0.898 && (HmassReg<75 || 110<HmassReg) && HmassReg<250 && mindPhiMETJet_dPhi>0.5 && naJets_Znn>=1 && hbhe && ecalFlag && cschaloFlag && hcallaserFlag && trackingfailureFlag && eebadscFlag && !isBadHcalEvent && hJet_puJetIdL[0]>0 && hJet_puJetIdL[1]>0 && HmassReg>0";
        return values;
    }
