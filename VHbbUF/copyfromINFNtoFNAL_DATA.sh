...
INFN="srm://stormfe1.pi.infn.it:8444/srm/managerv2?SFN=/cms/store/user/arizzi/Ntuple_Step1V42_Step2Tag_EDMV42_Step2_V6_MC/"
FNAL="srm://cmssrm.fnal.gov:8443/srm/managerv2?SFN=/11/store/user/lpchbb/apana/Ntuple_Step1V42_Step2Tag_EDMV42_Step2_V6_MC/"

CUT="${INFN#srm*\=}"

#voms-proxy-init -voms cms
lcg-ls -b -D srmv2 $INFN > COPYME.txt
sed -e "s@$CUT\(.*root\)@$INFN\1 $FNAL\1@" -i COPYME.txt
#sed -e "s@$CUT\(.*root\)@'$INFN\1' '$FNAL\1'@" -i COPYME.txt

echo "To start transferring, do:"
echo "srmcp -2 -streams_num=20 -copyjobfile=COPYME.txt"
