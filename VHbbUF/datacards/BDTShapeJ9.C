...
const TString g_varslice    = Form("slice(BDTtt_%i[ISYST], BDTvjlf_%i[ISYST], BDTzz_%i[ISYST], BDTregular_%i[ISYST], -0.6, -0.5, -0.3)", massH, massH, massH, massH);
...
const TString g_dcname      = "vhbb_Znn_$CHANNEL_8TeV.txt";
const TString g_wsname      = "vhbb_Znn_$CHANNEL_8TeV.root";
const TString g_rootname    = "vhbb_Znn_$CHANNEL_TH1.root";
...
const TString g_plotdir     = "plotsJ10/";
#endif
...
const double g_scalefactors_ZnunuHighPt[5] = {
    0.893, 0.662, 1.114, 1.199, 0.925
};
const double g_scalefactors_lnN_ZnunuHighPt[5] = {
    1.04, 1.24, 1.03, 1.08, 1.03
};

const double g_scalefactors_ZnunuMedPt[5] = {
    0.920, 0.378, 1.429, 1.795, 0.949
};
const double g_scalefactors_lnN_ZnunuMedPt[5] = {
    1.04, 1.25, 1.03, 1.06, 1.03
};

const double g_scalefactors_ZnunuLowPt[5] = {
    1.217, 0.795, 1.751, 1.303, 0.939
};
const double g_scalefactors_lnN_ZnunuLowPt[5] = {
    1.04, 1.22, 1.03, 1.06, 1.03
};

const double g_scalefactors_ZnunuLowCSV[5] = {
    1.000, 1.300, 1.000, 1.300, 1.000
};
...
void MakePlots(const EventsJ9 * ev, const TString var, 
               ...
               TString options="printStat:printCard:plotData:plotLog:plotSig")
{
    ...
    //hmc_exp_0->Add(hVH_0);  // FIXME: add VH as background?
#endif
    ...
    if (syst == "NONE") {
        ...
        if (!plotData) {  // be blind to the most sensitive bins
            for (Int_t i = TMath::Max(nbins_plot*0.75, nbins_plot-5.); i < nbins_plot+2; i++) {
        ...
        for (Int_t i = 0; i < hmc_exp->GetNbinsX()+2; i++) {
            if (hmc_exp->GetBinContent(i) > 1e-6) {  // use smaller tolerance?
                ...
                double binerror2 = (pow(hmc_exp->GetBinError(i), 2) +
                                    pow(0.05 * hWjLF->GetBinContent(i), 2) +
                                    pow(0.20 * hWjHF->GetBinContent(i), 2) +
                                    pow(0.05 * hZjLF->GetBinContent(i), 2) +
                                    pow(0.10 * hZjHF->GetBinContent(i), 2) +
...
void BDTShapeJ9(Int_t nbins=500, Int_t newnbins=16, Double_t rebinerrorf=0.35, bool pqrs=false, bool isControlRegion=false)
{
    ...
    for (Int_t ireg=begin; ireg<end; ireg++) {
        ...
        if (ireg==0) {  // signal region
            ...
            for (Int_t isyst = 0; isyst < nsyst; isyst++) {
                ev->set_scale_factors(&scalefactors[0]);
                ...
                TCut cutmc_test   = Form("2.0 * weightsHCP[%i] * (selectFlags[0][%i])", isyst, isyst);
        ...
        if (g_manyplots) {
            ...
            MakePlots(ev, "hJet_ptReg[1]", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H j2 p_{T} [GeV]", 13, 15, 210, 13, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            ...
            // Bkg specific BDT
            MakePlots(ev, Form("BDTtt_%i[0]", massH), cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "BDTtt", nbins, g_xlow, g_xup, newnbins, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:plotLog:plotSig");
            MakePlots(ev, Form("BDTvjlf_%i[0]", massH), cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "BDTvjlf", nbins, g_xlow, g_xup, newnbins, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:plotLog:plotSig");
            MakePlots(ev, Form("BDTzz_%i[0]", massH), cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "BDTzz", nbins, g_xlow, g_xup, newnbins, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:plotLog:plotSig");
            
            // QCD check
            MakePlots(ev, "METtype1corr.et/sqrt(METtype1corr.sumet)", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "Type-1 corr. pfMET/sqrt(sumET) [GeV]", 25, 0, 25, 25, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
    ...
    for (Int_t ireg=begin; ireg<end; ireg++) {
        ...
        if (ireg==0) {  // signal region
            ...
            for (Int_t isyst = 0; isyst < nsyst; isyst++) {
                ...
                Int_t newnbins1 = (!pqrs) ? 15 : 12;  // FIXME: currently it's hardcoded
                Double_t rebinerrorf1 = 0.35;
                MakePlots(ev, var, cutmc_test, cutdata_test, g_systematics[isyst], g_regions[ireg], "BDT", nbins, g_xlow, g_xup, newnbins1, rebinerrorf1, scalefactors_lnN, "printStat:printCard:writeRoot:!plotData:plotLog:plotSig");
                std::clog << "isyst " << isyst << ": DONE!\n" << std::endl;
    ...
    for (Int_t ireg=begin; ireg<end; ireg++) {
        ...
        if (ireg==0) {  // signal region
            ...
            for (Int_t isyst = 0; isyst < nsyst; isyst++) {
                ...
                //TCut cutmc_test   = Form("2.0 * weightsHCP[%i] * (selectFlags[0][%i])", isyst, isyst);
                TCut cutmc_test = Form("2.0 * efflumi * PUweight * triggerFlags[41] * 11299./12101. * (selectFlags[0][%i])", isyst);  // FIXME
                TCut cutdata_test = Form("selectFlags[0][%i]", isyst);
                
                Int_t newnbins1 = (!pqrs) ? 13 : 11;  // FIXME: currently it's hardcoded
                Double_t rebinerrorf1 = 0.35;
                MakePlots(ev, var, cutmc_test, cutdata_test, g_systematics[isyst], g_regions[ireg], "BDT", nbins, g_xlow, g_xup, newnbins1, rebinerrorf1, scalefactors_lnN, "printStat:printCard:writeRoot:!plotData:plotLog:plotSig");
                std::clog << "isyst " << isyst << ": DONE!\n" << std::endl;
        ...
        if (g_manyplots) {
            ...
            ev->set_scale_factors(&scalefactors[0]);
            //TCut cutmc_ctrl = Form("weightsHCP[0] * (selectFlags[%i][0])", ireg);
            TCut cutmc_ctrl = Form("efflumi * PUweight * triggerFlags[41] * 11299./12101. * (selectFlags[%i][0])", ireg);  // FIXME
            TCut cutdata_ctrl = Form("(selectFlags[%i][0])", ireg);
            TCut cutmass_ctrl = "";
            if (ireg==0) {  // signal region
                cutmc_ctrl = Form("2.0 * weightsHCP[0] * (selectFlags[%i][0])", ireg);
            ...
            MakePlots(ev, "hJet_ptReg[0]", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H j1 p_{T} [GeV]", 20, 15, 315, 20, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "hJet_ptReg[1]", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "H j2 p_{T} [GeV]", 13, 15, 210, 13, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "deltaR(hJet_eta[0], hJet_phi[0], hJet_eta[1], hJet_phi[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "#Delta R(j1,j2)", 14, 0, 3.5, 14, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "abs(hJet_eta[0]-hJet_eta[1])", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "|#Delta #eta|(j1,j2)", 14, 0, 3.5, 14, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
            MakePlots(ev, "min(abs(deltaPullAngle),pi)", cutmc_ctrl, cutdata_ctrl, g_systematics[0], g_regions[ireg], "|#Delta #theta_{pull}|(j1,j2)", 16, 0, 3.2, 16, rebinerrorf, scalefactors_lnN, "!printStat:!printCard:!writeRoot:plotData:!plotLog:plotSig");
...
/*
    ...
    for (Int_t ireg=begin; ireg<TMath::Min(end,3); ireg++) {
        /// Initialize rebinning "bookmarks" to negative values
        /// These global variables that need to be reset when we switch channel.
        firstN = -1, lastN = -1;  // for single BDT
        firstN1 = -1, firstN2 = -1, firstN3 = -1, firstN4 = -1; // for PQRS BDT
        ...
        if (ireg==0) {  // signal region
            ...
            for (Int_t isyst = 0; isyst < nsyst; isyst++) {
                ...
                Int_t newnbins1 = (!pqrs) ? 11 : 10;  // FIXME: currently it's hardcoded
                Double_t rebinerrorf1 = 0.35;
                MakePlots(ev, var, cutmc_test, cutdata_test, g_systematics[isyst], g_regions[ireg], "BDT", nbins, g_xlow, g_xup, newnbins1, rebinerrorf1, scalefactors_lnN, "printStat:printCard:writeRoot:!plotData:plotLog:plotSig");
                std::clog << "isyst " << isyst << ": DONE!\n" << std::endl;
