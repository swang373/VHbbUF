...
#ifndef HZZ2L2VNAMES
    "ZH_hbb", "WH_hbb", "ZH_hinv", "Wj0b", "Wj1b", "Wj2b", "Zj0b", "Zj1b", "Zj2b", "TT", "s_Top", "VVLF", "ZZ", "WZ", "QCD"
#else
    "ZH_SM", "WH_SM", "zh1252lmet", "Wj0b", "Wj1b", "Wj2b", "Zj0b", "Zj1b", "Zj2b", "TT", "s_Top", "VVLF", "zz2l2nu", "wz3lnu", "QCD"
#endif
};
                  ...
                  const Int_t p, const Int_t up, const Int_t down)
{
    ...
    if (g_systematics[up] == "CMS_vhbb_stat$PROCESS_$CHANNEL_8TeVUp") {
        ...
        for (UInt_t ibin=1; ibin<nbins+1; ibin++) {
            ...
            if (h_statUp->Integral() < 1e-6)  h_statUp->SetBinContent(ibin, 1e-6);
            if (h_statDown->Integral() < 1e-6)  h_statDown->SetBinContent(ibin, 1e-6);
...
void MakeWorkspace(const TString& channel, const TString& strategy)
{
    ...
    // nominal
    TString ratestr = "rate        ";
    for (Int_t p=0; p < jmax+1; p++) {
        const TString& process = g_processes[p];
        h = (TH1F *) input->Get(channel + "/" + process);
        ratestr += Form("%10.3f ", h->Integral());
        dh = new RooDataHist(process, "", *obs, h);
        ws->import(*dh);
        delete dh;
        delete h;
    }
    gSystem->Exec(Form("sed -i 's/rate        .*/%s/' %s ", ratestr.Data(), dcname.Data()));
